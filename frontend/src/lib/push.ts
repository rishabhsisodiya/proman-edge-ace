 ...
 * notification permission, registers the FCM service worker, obtains a
 * device token, and POSTs it to the backend (`/users/me/push-tokens`).
 * Silently no-ops — never throws — if Firebase isn't configured yet, the
 * browser doesn't support push, or the user denies/ignores the permission
 * prompt. Safe to call on every page load; the backend upserts on token, so
 * re-registering the same browser is a no-op there too.
 */
